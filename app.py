...
#             "Color": "#2ca02c" if prediction == 1 else "#d62728"
#         })
...
#         col2.metric("Sentiment", f"{'Positive' if analysis['sentiment'] > 0 else 'Negative' if analysis['sentiment'] < 0 else 'Neutral'}")
#         col3.metric("Subjectivity", f"{analysis['subjectivity']:.1f}/1.0")
...
#             with pred_cols[i]:
#                 st.markdown(f"""
#                 <div style='background-color:#f8f9fa;border-radius:10px;padding:10px;text-align:center;
#                             border-left:5px solid {result["Color"]};margin-bottom:10px;'>
#                     <h4>{result['Model']}</h4>
...
#                     border:2px solid {"#2ca02c" if overall == "REAL" else "#d62728"};margin-top:20px;'>
#             <h2>Overall Verdict: <span style='color:{"#2ca02c" if overall == "REAL" else "#d62728"};'>
#             {overall}</span></h2>
#             <p>{len([r for r in results if r['Prediction'] == overall])} out of 4 models agree</p>
#         </div>
#         """, unsafe_allow_html=True)
...
#     **4. Source Assessment:**
#     - Cross-references with known credible sources
#     - Evaluates author credibility when available
#     - Checks for consistency with established facts
...
#     🔍 Fake News Detection System v2.1 | Combating misinformation through AI
# </div>
# """, unsafe_allow_html=True)
